...
(function () {
  ...
  /** @ngInject */
  function baSidebar($timeout, baSidebarService, baUtil, layoutSizes, Auth) {
    var jqWindow = $(window);
    return {
      restrict: 'E',
      ...
      link: function(scope, el) {
        ...
        function _showSidebarItem(item) {
          let type = typeof item.authenticate;
          return (type === 'undefined') ||
            (type === 'boolean' && item.authenticate && Auth.isLoggedIn()) ||
            (type === 'boolean' && !item.authenticate && !Auth.isLoggedIn()) ||
            (type === 'string' && Auth.hasSpecificRole(item.authenticate));
        }

        scope.showSidebarItem = _showSidebarItem;
      }
    };
  }
